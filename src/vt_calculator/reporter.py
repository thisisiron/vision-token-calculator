...
class Reporter:
    ...
    def print(self, result: dict, model_name: str, image_source: str = None) -> None:
        ...
        # Title
        grid.add_row(Panel("[bold cyan]VISION TOKEN ANALYSIS REPORT[/bold cyan]", box=box.DOUBLE, expand=False))
        ...
        grid.add_row(Panel(model_table, title="[bold]MODEL INFO[/bold]", border_style="blue", box=box.ROUNDED))
        ...
        image_table.add_row("Original Size (H x W)", f"{result['image_size'][0]} x {result['image_size'][1]}")
        image_table.add_row("Resized Size (H x W)", f"{result['resized_size'][0]} x {result['resized_size'][1]}")
        grid.add_row(Panel(image_table, title="[bold]IMAGE INFO[/bold]", border_style="green", box=box.ROUNDED))
        ...
        if "grid_size" in result:
            patch_table.add_row("Grid Size (H x W)", f"{result['grid_size'][0]} x {result['grid_size'][1]}")
        patch_table.add_row(
            "Number of Patches",
            f"{result['number_of_image_patches']} {'(global patch)' if result['has_global_patch'] else ''}"
        )
        grid.add_row(Panel(patch_table, title="[bold]PATCH INFO[/bold]", border_style="yellow", box=box.ROUNDED))
        ...
        if items_to_show:
            ...
            grid.add_row(Panel(token_info_table, title="[bold]TOKEN INFO[/bold]", border_style="magenta", box=box.ROUNDED))

            # TOKEN FORMAT
            format_panel = Panel(
                Text(result['image_token_format'], style="bold white", justify="center"),
