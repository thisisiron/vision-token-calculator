...
def print_file_processing_result(
    filename: str,
    current: int,
    total: int,
    success: bool,
    ...
    """
    Print processing result for a single file with consistent formatting.
    ...
    """
    status_prefix = f"[{current}/{total}] Processing: {filename}"

    if success and token_count is not None:
        # Format token count with comma separators for readability
        formatted_tokens = f"{token_count:,}"
        print(f"{status_prefix} ✓ ({formatted_tokens} tokens)")
    elif not success and error:
        print(f"{status_prefix} ✗ (Error: {error})")
    else:
        # Fallback for unexpected cases
        print(f"{status_prefix} ✗ (Unknown error)")
