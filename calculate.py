...
def process_directory(directory_path: str, model_path: str):
    ...
    for i, image_file in enumerate(image_files, 1):
        ...
        try:
            ...
            print_file_processing_result(
                filename, i, len(image_files), True, token_count
            )
        except Exception as e:
            print_file_processing_result(
                filename, i, len(image_files), False, error=str(e)
            )
            processed_files.append(
                {"filename": filename, "size": None, "tokens": None, "error": str(e)}
            )

    # Calculate statistics
    stats = {
        "total_processed": len(processed_files),
        "total_failed": sum(1 for f in processed_files if f.get("error") is not None),
        "average_tokens": calculate_mean(token_counts),
        "min_tokens": min(token_counts),
        "max_tokens": max(token_counts),
        "std_deviation": calculate_stdev(token_counts),
        "processed_files": processed_files,
        "failed_files": [f for f in processed_files if f.get("error") is not None],
    }
